...
2. **Opt-in startup and transactional enablement (SPEC §§6, 10).** Session startup establishes the ordinary active-tool baseline and leaves Godmode off without changing model or thinking level. Explicit TUI `/godmode` enablement checks trust, configuration, capabilities, models, registration, preflight, and tools; failures roll back partial changes to `off`.
