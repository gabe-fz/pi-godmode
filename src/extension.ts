...
import { appendWorkflowSnapshot, createWorkflowOperationIdentity } from "./session-ledger.ts";
...
export default function godmodeExtension(pi: ExtensionAPI): void {
  ...
  const persistWorkflowRecord = (next: WorkflowRecord, timestamp: string, operationId?: string): WorkflowRecord => {
    const ctx = requireContext();
    const previousInspection = workflowRecord?.primaryInspection;
    const persisted = appendWorkflowSnapshot(pi, ctx.sessionManager, next, timestamp, undefined, operationId);
  ...
  const persistWorkflowTransition = (to: WorkflowPhase, reason: string, reference: string): WorkflowRecord => {
    ...
    const persisted = persistWorkflowRecord(next, timestamp, createWorkflowOperationIdentity("workflow-transition", {
  ...
  const mode = new GodmodeMode({
    ...
    persistScaleAdmission: (admission: ScaleAdmission): WorkflowRecord => {
      ...
      // Admission IDs, nonces, and timestamps are regenerated by a retry;
      // identity is the exact current inspection admission, not those trusted
      // audit fields. The persisted candidate is still required to equal the
      // first snapshot exactly before this callback adopts it.
      const operationId = createWorkflowOperationIdentity("scale-admission", {
        workItemId: current.workItemId,
        inspectionId: current.primaryInspection?.id,
        diffFingerprint: current.primaryInspection?.diffFingerprint,
      });
      return persistWorkflowRecord(next, timestamp, operationId);
    },
    bindScaleAdmission: (admissionId: string, runId: string): WorkflowRecord => {
      ...
      const operationId = createWorkflowOperationIdentity("scale-admission-bind", {
        workItemId: current.workItemId,
        admissionId,
        runId,
      });
